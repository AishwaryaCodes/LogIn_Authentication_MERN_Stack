...
export const registerUser = asyncHandler(async (req, res) => {
   ...
   //validations
   if(!name || !email || !password) res.status(400).json({ message: "Please fill all fields" });
   

   //Check PW length // check uppercase - Later
   if(password.length < 6) return res.status(404).json({ message: "Password must be at least 6 Character."});
   ...
   // send back the user and token in responce to the client
...
export const loginUser = asyncHandler(async (req, res) => {
    ...
    //Validations
    if(!email || !password) res.status(400).json({ message: "Please fill all fields" }); 

    const userExists = await User.findOne({email});

    if (!userExists) res.status(404).json({ message: "User not Found" }); 
    
    //check if PW match the Pw(hashed pw) in database.
    ...
    if(userExists && isMatch) {
        ...
        //send back the user and token in the responce to the client
        res.status(200).json({
            _id,
            name,
            email,
...
//Get User - get user details from  token
...
//get user details fron the token - protect middleware
...
export const userLoginStatus = asyncHandler(async (req, res) => {
    ...
    if(decoded) res.status(200).json(true);
