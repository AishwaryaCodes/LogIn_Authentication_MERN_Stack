...
export const protect = asyncHandler(async (req, res, next) => {
    try{
        ...
        if(!token) res.status(401).json({message: "Not authorized, please log in"});
...
// Creator middleware creatorMiddleware
...
export const verifiedMiddleware = asyncHandler(async (req, res, next) => {
    ...
    res.status(403).json({message: "please verify email address."})
});